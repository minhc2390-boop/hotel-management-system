				...
				<div class="content-inner">
					<div class="page-head">
						<div>
							...
							<p class="page-desc">Quản lý các dịch vụ và mức giá áp dụng
								cho khách lưu trú.</p>
						</div>
						<div class="page-actions">
							<a class="btn btn-outline"
								href="<%=request.getContextPath()%>/admin/service-book.jsp">Dịch
								vụ đã gọi</a>
							<%
							if (isAdmin) {
							%><a class="btn btn-primary"
								href="<%=request.getContextPath()%>/services?action=add">＋
								Thêm mới</a>
							<%
							}
							%>
					...
					<section class="surface">
							...
							<table>
								...
								<tbody>
									...
									%><tr>
										...
										<td><%=s.getUnit() != null ? s.getUnit() : "Lượt"%></td>
										<td><%=money.format(s.getPrice())%></td>
										<td><%=s.getDescription() != null ? s.getDescription() : "-"%></td>
										<td>
											<%
											if ("Active".equals(s.getStatus()) || s.getStatus() == null) {
											%><span
											class="status success">Đang hoạt động</span>
											<%
											} else {
											%><span class="status danger">Tạm ngưng</span>
											<%
											}
											%>
										</td>
						...
						%><div class="empty">
							<strong>Chưa có dịch vụ</strong>Hãy thêm dịch vụ đầu tiên.
						</div>
						<%
						}
						%>
