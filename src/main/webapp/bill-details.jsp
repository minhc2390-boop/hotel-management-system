  ...
  <body>
    <header class="public-header">
      ...
      <nav class="public-nav">
        ...
        <% if (admin) { %>
        <a href="<%= request.getContextPath() %>/bills?action=list">Hóa đơn</a>
        <% } else { %>
        ...
        <div>
          <section class="surface">
            <div class="surface-head">
              <div><h2 class="surface-title">Hạng mục thanh toán</h2></div>
            </div>
                ...
                <tbody>
                  ...
                  <tr>
                    <td class="table-strong">
                      <% if (bd.getRoomId() != null) { %>Phòng <%= bd.getRoom() != null ? bd.getRoom().getRoomNumber() : bd.getRoomId() %>
<% } else if (bd.getServiceId() != null) { %> <%= bd.getService() != null ? bd.getService().getName() : bd.getServiceId() %>
                      <% } else { %>Khác<% } %>
                    </td>
          ...
          <section class="surface surface-pad" style="margin-top: 16px">
            ...
            <form action="<%= request.getContextPath() %>/bills" method="post">
              ...
              <div class="form-actions"><button class="btn btn-primary" type="submit">Thêm dịch vụ</button></div>
            </form>
          </section>
          <% } %>
        </div>
        <aside class="surface surface-pad">
          ...
          <div class="checkout-line">
            <span>Ngày tạo</span>
            <strong><%= dt.format(bill.getCreatedAt()) %></strong>
          </div>
          <div class="checkout-line">
            <span>Nhận phòng</span>
            <strong><%= d.format(bill.getCheckInDate()) %></strong>
          </div>
          <div class="checkout-line">
            <span>Trả phòng</span>
            <strong><%= d.format(bill.getCheckOutDate()) %></strong>
          </div>
          <h2 class="form-title" style="margin-top: 22px">Khách hàng</h2>
          <div class="checkout-line">
            <span>Họ tên</span>
            <strong><%= bill.getUser().getFullName() %></strong>
          </div>
          <div class="checkout-line">
            <span>Email</span>
            <strong><%= bill.getUser().getEmail() %></strong>
          </div>
          <div class="checkout-line">
            <span>Điện thoại</span>
            <strong><%= bill.getUser().getPhone() != null ? bill.getUser().getPhone() : "Chưa có" %></strong>
          </div>
          <div style="display: grid; gap: 8px; margin-top: 18px">
            <% if ("Unpaid".equals(bill.getStatus()) && admin) { %>
            <a class="btn btn-success" href="<%= request.getContextPath() %>/bills?action=pay&id=<%= bill.getId() %>">
              Xác nhận thanh toán
            </a>
            <% } %>
<% if ("Unpaid".equals(bill.getStatus())) { %>
