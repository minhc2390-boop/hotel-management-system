          ...
          <div class="content-inner">
            <div class="page-head">
              <div>
                ...
                <h1 class="page-title">Chào buổi sáng, <%= currentUser.getFullName() %>!</h1>
                <p class="page-desc">Dưới đây là tình hình hoạt động khách sạn hôm nay.</p>
              </div>
              <div class="page-actions">
                <a class="btn btn-primary" href="<%= request.getContextPath() %>/admin/booking-form.jsp">＋ Tạo đặt phòng</a>
              </div>
            </div>
            ...
            <div class="stat-grid">
              <div class="stat-card">
                ...
                <div class="stat-value"><%= totalRooms > 0 ? Math.round(bookedRooms * 100.0 / totalRooms) : 0 %>%</div>
                <div class="stat-change">↗ Theo dữ liệu phòng hiện tại</div>
              </div>
              <div class="stat-card">
                <div class="stat-label">Phòng đang thuê</div>
                <div class="stat-value"><%= bookedRooms %></div>
                <div class="stat-change">Trong tổng số <%= totalRooms %> phòng</div>
              </div>
              <div class="stat-card">
                <div class="stat-label">Phòng trống</div>
                <div class="stat-value"><%= availableRooms %></div>
                <div class="stat-change">Sẵn sàng nhận khách</div>
              </div>
              <div class="stat-card">
            ...
            <div class="dashboard-grid">
              <section class="surface">
                <div class="surface-head">
                  <div>
                    <h2 class="surface-title">Doanh thu 7 ngày gần đây</h2>
                    <p class="surface-subtitle">Biểu đồ minh họa giao diện; số tổng lấy từ backend.</p>
                  </div>
                  <a href="<%= request.getContextPath() %>/admin/profits.jsp" class="table-primary">Xem chi tiết</a>
                </div>
              ...
              <section class="surface">
                <div class="surface-head">
                  <div>
                    <h2 class="surface-title">Phân bố phòng hôm nay</h2>
                    <p class="surface-subtitle">Cập nhật trực tiếp từ dữ liệu phòng</p>
                  </div>
                </div>
                <ul class="mini-list">
                  <li>
                    <div class="mini-main">
                      <strong>Phòng đang thuê</strong>
                      <span>Khách đang lưu trú</span>
                    </div>
                    <span class="status info"><%= bookedRooms %></span>
                  </li>
                  <li>
                    <div class="mini-main">
                    ...
                    <span class="status success"><%= availableRooms %></span>
                  </li>
                  <li>
                    <div class="mini-main">
                      <strong>Đang bảo trì</strong>
                      <span>Cần xử lý kỹ thuật</span>
                    </div>
                    <span class="status warning"><%= maintenanceRooms %></span>
                  </li>
                  <li>
                    <div class="mini-main">
                      <strong>Tài khoản hệ thống</strong>
                      <span>Nhân viên và khách hàng</span>
                    </div>
                    <span class="status purple"><%= totalUsers %></span>
                  </li>
                  <li>
                    <div class="mini-main">
                      <strong>Tổng hóa đơn</strong>
                      <span>Đã phát sinh trong hệ thống</span>
                    </div>
                    <span class="status info"><%= totalBills %></span>
                  </li>
                </ul>
              </section>
            </div>
            ...
            <section class="surface" style="margin-top: 18px">
              <div class="surface-head">
                <div>
                  <h2 class="surface-title">Đặt phòng gần đây</h2>
                  <p class="surface-subtitle">Dữ liệu thật từ danh sách hóa đơn/đặt phòng</p>
                </div>
                <a class="table-primary" href="<%= request.getContextPath() %>/bills?action=list">Xem tất cả</a>
              </div>
                  ...
                  <tbody>
                    <%
                        ...
                        for (int i = 0;i<limit;i++) {
                            Bill b = recentBills.get(i);
                    %>
                    <tr>
                      <td class="table-primary">#<%= b.getId() %></td>
                      <td>
                        <span class="table-strong"><%= b.getUser().getFullName() %></span>
                        <br />
                        <span class="text-muted"><%= b.getUser().getEmail() %></span>
                      </td>
                      <td><%= dateTime.format(b.getCreatedAt()) %></td>
                      ...
                      <td>
                        <a
                          ...
                          href="<%= request.getContextPath() %>/bills?action=detail&id=<%= b.getId() %>"
                        >
                          ›
                        </a>
                      </td>
              ...
              <% } else { %>
              <div class="empty">
                <strong>Chưa có đặt phòng gần đây</strong>
                Dữ liệu sẽ hiển thị khi hệ thống có giao dịch.
              </div>
              <% } %>
            </section>
          </div>
